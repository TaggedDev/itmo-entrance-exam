...
CATEGORY_KEYWORDS = {
    "payments": ["оплат", "платеж", "списан", "подписк", "возврат", "деньги", "чек"],
    "auth": ["вход", "логин", "парол", "код", "email", "аккаунт", "2fa"],
    "feedback": ["предлож", "улучш", "фича", "идея", "хочу", "неудобно"],
    "legal": ["юрид", "суд", "претензи", "персональн", "данные", "договор"],
}
...
def generate_mock_response(category: str, risk_level: str, contexts: list[dict[str, object]]) -> str:
    if risk_level == "high":
        return (
            ...
            "Не отправляйте ответ пользователю без проверки специалистом и сверки с внутренними регламентами."
        )
